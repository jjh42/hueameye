...
class RapleafApi:
	...
	def __init__(self, apiKey):
		self.basePath = 'https://' + RapleafApi.host + RapleafApi.basePath + '?api_key=%s' % (apiKey)
    ...
    the error code and error body are accessible from the exception raised
		...
		if show_available:
			path += '&show_available'
		json_response = fetch(path, headers = RapleafApi.headers)
		if 200 <= json_response.status_code < 300:
			if json_response.content:
				return json.JSONDecoder().decode(json_response.content)
			else:
				return {}
		else:
			raise Exception(json_response.status_code, json_response.content)
